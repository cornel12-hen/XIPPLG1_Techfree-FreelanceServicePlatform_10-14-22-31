...
<html lang="en">
  ...
  <body class="bg-gray-100 lg:pt-0 max-lg:pt-20">
      ...
      <div class="max-w-7xl mx-auto px-4">
        <div class="flex justify-between items-center py-4">
          ...
          <div class="hidden lg:flex items-center space-x-6">
            <div class="space-x-4">
              ...
              <a href="{{ route('freelancer.dashboard') }}" class="text-pkk font-semibold">Dashboard</a>
            </div>
            <a href="#CTA">
        ...
        <div id="mobile-menu" class="fixed top-0 right-0 h-full w-48 bg-white shadow-lg transform translate-x-full transition-transform duration-300 ease-in-out lg:hidden z-40">
          <div class="flex flex-col items-start space-y-4 py-8 px-6">
            <a href="{{ route('freelancer.explore') }}" class="block hover:text-pkk">Explore Jobs</a>
            <a href="#" class="block hover:text-pkk">My Jobs</a>
            <a href="{{ route('freelancer.dashboard') }}" class="block hover:text-pkk">Dashboard</a>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
    ...
    <div class="w-full flex h-screen" x-data="{ selectedJob: null, selectedJobDetails: null }">
      ...
      <div class="w-1/3 border-r overflow-y-auto bg-gray-100">
          <h2 class="text-lg font-bold p-4 bg-gray-200">My Jobs</h2>
  
          {{-- <!-- Debugging -->
          <p class="text-sm text-gray-500 p-2">Selected Job ID: <span x-text="selectedJob"></span></p>  --}}
          
          @foreach($jobs as $job)
              <div class="p-4 border-b cursor-pointer hover:bg-gray-300" 
                   x-on:click="selectedJob = {{ $job->id }}; selectedJobDetails = {{ json_encode($job->workJob) }}">
                  <h3 class="font-semibold">{{ $job->workJob->name }}</h3>
                  <p class="text-sm text-gray-600">Client: {{ $job->workJob->client->name }}</p>
              </div>
          @endforeach
      </div>
          ...
          <template x-if="selectedJobDetails">
              <div>
                  ...
                  <div class="mt-4">
                      ...
                      <p class="text-sm text-gray-600" x-text="selectedJobDetails.client.name"></p>
                  </div>
