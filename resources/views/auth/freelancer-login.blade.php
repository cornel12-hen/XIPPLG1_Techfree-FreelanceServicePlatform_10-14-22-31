...
<html lang="en">
  <head>
    ...
    <title>Freelancer Log In - Techfree</title>
    ...
    <section class="grid md:grid-cols-10 grid-rows-1 max-w-7xl mx-auto">
      ...
      <div class="md:col-span-6 md:h-screen h-fit pb-10 bg-white flex flex-col md:pl-20 md:pr-20 pt-12 pl-8 pr-8 font-sans gap-6">
        <div>
          ...
          <p class="text-lg">Please log in first</p>
        </div>

        <form action="{{ route('freelancer.login') }}" method="POST" class="flex flex-col">
          @csrf
          <label for="email">Email</label>
          <input class="w-full mb-2 mt-[1px] border-2 border-dark rounded-md h-10 p-2" type="text" name="email" value="{{ old('email') }}" />

          <label for="password">Password</label>
          <input class="w-full mb-4 mt-[1px] border-2 border-dark rounded-md h-10 p-2" type="password" name="password" />
          @error('email')
              @if (str_contains($message, 'email'))
                  <span class="text-red-500 w-full text-sm mb-4 mt-[-12px]">Email atau Password salah!</span>
              @endif
          @enderror

          <button type="submit" class="bg-dark w-full h-10 rounded-md text-white font-bold mb-5 pb-0.5">Log In</button>
        ...
        <p class="text-center">Don't have an account? <a href="register" class="text-pkk">Register</a></p>
      </div>
    </section>
